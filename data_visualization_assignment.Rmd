...
dt <- read_delim(
  "/Users/cmdb/qb25-answers/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct",
...
# answer here
# I notice the shape of the distribution is highly grouped towards 0 and that most of the genes are within 2 TPM from the range of 0 to 100. No it is not specific to the liver, other tissues also share the same pattern.
```
...
ggplot(dt, aes(x = Liver+1)) +
  ...
  scale_color_manual(values = c("cyan", "lightpink", "black")) +
  theme_classic()
...
ggplot(dt, aes(x =`Liver`+1 , y= `Heart - Left Ventricle`+1),colors=is_mitochondrial) +
  geom_point(alpha = 0.2) +
  geom_abline(slope = 1, lty = "dashed", color = "red") +
    scale_x_log10() +
  scale_y_log10() +
